...
export class WebSocketMessageBoardComponent implements OnInit {
  ...
  constructor( 
    ...
    if (this.data.event === 'mint-nft') {
      ...
      this.displayData = [
        {id: 0, message: this.data.data.bChainSymbol === 'ETH' ? 'Validating confirmed payment' : 'Validating payment transaction', status: 'processing'},
        {id: 1, message: this.data.data.bChainSymbol === 'ETH' ? 'Deploying contract with metadata' : 'Uploading token metadata', status: 'pending'},
